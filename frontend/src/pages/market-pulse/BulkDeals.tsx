...
import { fetchBulkDealsStart } from '@/store/slices/marketPulseSlice'
...
export default function BulkDeals() {
  ...
  const { items: deals, total, page, limit, status, error } =
    useSelector((s: RootState) => s.marketPulse.bulkDeals)

  const loading = status === 'loading'

  // Fetch when page / limit change
  useEffect(() => {
    dispatch(fetchBulkDealsStart({ page, limit }))
  }, [dispatch, page, limit])

  const handleRetry = () => dispatch(fetchBulkDealsStart({ page, limit }))
    ...
    <div className="min-h-screen bg-background font-sans select-none">
      <div className="max-w-[1200px] mx-auto px-6 py-6 select-none">
        ...
        <div className="text-xs text-textSecondary/70 mb-2 flex items-center gap-1.5">
          ...
          <ChevronRight className="size-3" />
          <span className="text-accent font-medium">Bulk Deals</span>
        </div>
          ...
          <div className="bg-surface border border-border/40 rounded-xl overflow-hidden shadow-xs mb-8">
              ...
              {loading ? (
                ...
                <Empty className="py-12 border-0">
                  <EmptyHeader>
                    ...
                    <EmptyTitle className="text-textPrimary font-semibold">No bulk deals found</EmptyTitle>
            ...
            {!loading && total > 0 && (
              <PaginationBar
                ...
                page={page}
                limit={limit}
                onPageChange={(p) => dispatch(fetchBulkDealsStart({ page: p, limit }))}
                onLimitChange={(l) => dispatch(fetchBulkDealsStart({ page: 1, limit: l }))}
